...
*
*  Copyright 2015 World Light Information Limited and
...
typedef struct
{
    const char* mName;          /* Entity name */
    int         mUcs;           /* Unicode code point */
} EntityData;


// MathML entities from http://www.w3.org/2003/entities/2007/htmlmathml-f.ent
static const EntityData gpEntities[] = 
{
    {"AElig",            0xC6},
    {"AMP",              0x26},
    {"Aacute",           0xC1},
...
static const int gEntityCount = sizeof(gpEntities) / sizeof(gpEntities[0]);
...
mathml_entity_callback(const char* name)
{
    ...
    // Perform a binary search for the name
    high = gEntityCount - 1;
    while (low <= high)
    {
        // Locate current search record
        mid = (low + high) / 2;

        // Perform comparison
        result = strcmp(name, gpEntities[mid].mName);
        if (result < 0)
        {
            // target < item, search [low..mid-1]
            high = mid - 1;
        }
        else if (result > 0)
        {
            // target > item, search [mid+1..high]
            low = mid + 1;
        }
        else
        {
            // Exact match found, return Unicode code point
            return gpEntities[mid].mUcs;
        }
    }
